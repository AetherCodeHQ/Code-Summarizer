...
> Code-Summarizer - Open source tool by AetherCodeHQ

`ai` `cli` `developer-tools` `golang` `summarization`

## What is it?

**Code Summarizer** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Code-Summarizer.git
cd Code-Summarizer
go build -o Code-Summarizer .
```

### Run
```bash
./Code-Summarizer --help
```
...
```bash
# Basic usage
./Code-Summarizer --path ./target

# With options
./Code-Summarizer --path ./target --format json --output report.json

# Verbose mode
./Code-Summarizer --path ./target --verbose
```
...
### Basic scan
```bash
./Code-Summarizer --path ./my-project
```

### JSON report
```bash
./Code-Summarizer --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Code Summarizer
  run: ./Code-Summarizer --path . --format json --output report.json
```
